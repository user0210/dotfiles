...
#command='java -jar -Xms128M -Xmx3000M -XX:+UseG1GC -XX:+UnlockExperimentalVMOptions -XX:MaxGCPauseMillis=100 -XX:+DisableExplicitGC -XX:TargetSurvivorRatio=90 -XX:G1NewSizePercent=50 -XX:G1MaxNewSizePercent=80 -XX:G1MixedGCLiveThresholdPercent=35 -XX:+AlwaysPreTouch -XX:+ParallelRefProcEnabled -Dusing.aikars.flags=mcflags.emc.gs paperclip.jar nogui'

command='java -Xms2600M -Xmx2600M -XX:+UseG1GC -XX:+ParallelRefProcEnabled -XX:MaxGCPauseMillis=200 -XX:+UnlockExperimentalVMOptions -XX:+DisableExplicitGC -XX:+AlwaysPreTouch -XX:G1NewSizePercent=30 -XX:G1MaxNewSizePercent=40 -XX:G1HeapRegionSize=8M -XX:G1ReservePercent=20 -XX:G1HeapWastePercent=5 -XX:G1MixedGCCountTarget=4 -XX:InitiatingHeapOccupancyPercent=15 -XX:G1MixedGCLiveThresholdPercent=90 -XX:G1RSetUpdatingPauseTimePercent=5 -XX:SurvivorRatio=32 -XX:+PerfDisableSharedMem -XX:MaxTenuringThreshold=1 -Dusing.aikars.flags=https://mcflags.emc.gs -Daikars.new.flags=true -jar paperclip.jar nogui'

#command='java -jar -Xms2600M -Xmx2600M paperclip.jar nogui'
...
config() {
	...
	if [ -f "paper.yml" ]; then
		...
		# custom settings
		# enable restore TNT, Rails and Carpet duping. enable bedrock and End portal block destruction. headless pistons.
		sed -i "s/allow-permanent-block-break-exploits: false/allow-permanent-block-break-exploits: true/g" $basedir/paper.yml
		sed -i "s/allow-piston-duplication: false/allow-piston-duplication: true/g" $basedir/paper.yml
		sed -i "s/allow-headless-pistons: false/allow-headless-pistons: true/g" $basedir/paper.yml
...
update() {
	echo "download version and rename to paperclip.jar ..."
}
